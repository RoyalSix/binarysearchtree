...
//***********************************************************
// Below are new member functions to implement
//***********************************************************

//Find the maximum among all values stored in the tree.
int Tree ::FindMaximum()
{
	max = FindMaximumOfTree(Root);
	return max;
}

int Tree::FindMaximumOfTree(TreeNode* SubtreeRoot)
{
	...
	if (SubtreeRoot->Right == nullptr)
		...
		(FindMaximumOfTree(SubtreeRoot->Right));
}

//Find the average of all values stored in the tree.
int Tree :: FindAverage()
{
	int avg = FindAverageOfTree(Root);
	return avg;
}

float Tree::FindSize()
{
	size = FindSizeOfTree(Root);
	return size;
}


float Tree::FindSizeOfTree(TreeNode* SubtreeRoot)
{  
  
  if (SubtreeRoot==NULL) 
    return 0;
  else    
    return(FindSizeOfTree(SubtreeRoot->Left) + 1 + FindSizeOfTree(SubtreeRoot->Right));  
}


float Tree::TotalOfTree(TreeNode* SubtreeRoot)
{
	if (SubtreeRoot == Root)
		total = 0;
	...
	{ 
		total = total + SubtreeRoot->Value;
		int x = (TotalOfTree(SubtreeRoot->Left) + TotalOfTree(SubtreeRoot->Right));
		return total;
	}
}


float Tree::FindAverageOfTree(TreeNode* SubtreeRoot)
{
	...
	{
		return(TotalOfTree(Root)/FindSizeOfTree(Root));
	}
...
int Tree :: FindHeight()
{
	height = FindHeightOfSubtree(Root);
	return height-1;
}

//Find the height of a subtree whose root node is pointed to 
//	by the pointer SubtreeRoot.
int Tree :: FindHeightOfSubtree(TreeNode * SubtreeRoot)
{
	if (SubtreeRoot == nullptr)
		return 0;
	else 
	{
	int size1 = FindHeightOfSubtree(SubtreeRoot->Left);
	int size2 = FindHeightOfSubtree(SubtreeRoot->Right);
